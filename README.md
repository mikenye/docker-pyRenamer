...
[![pyRenamer logo](https://launchpadlibrarian.net/17610686/pyrenamer_64.png)](https://github.com/SteveRyherd/pyRenamer)[![pyRenamer](https://dummyimage.com/200x60/ffffff/575757&text=pyRenamer)](https://github.com/SteveRyherd/pyRenamer)

pyRenamer is an application for mass renaming files.

You can rename files using patterns, substitutions, insert or delete text, or even rename files manually. You can also rename images using their EXIF tags and music using their internal tags.
...
Launch the pyRenamer docker container with the following command:
```
docker run -d \
    --name=pyrenamer \
    -p 5800:5800 \
    -v /path/to/config:/config:rw \
    -v /path/to/files:/storage:rw \
    mikenye/pyrenamer
```

Where:
  - `/path/to/config`: This is where the application stores its configuration, log and any files needing persistency.
  - `/path/to/files`: This location contains files for pyRenamer to operate on.

Browse to `http://your-host-ip:5800` to access the pyRenamer GUI. Your files will be located under `/storage`.
...
|`/storage`| rw | This location contains files from your host that need to be accessible by the application. Should be 'rw' so pyRenamer can re-name your files.|
...
```
docker exec -ti pyrenamer bash
```

## Support or Contact

Having troubles with the container or have questions?  Please [create a new issue](https://github.com/mikenye/docker-pyrenamer/issues).
